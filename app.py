...
class RestaurantApp:
    ...
    def show_login(self):
        ...
        tk.Label(card, text="🍽", fg=MAROON, bg=WHITE, font=("Arial", 42, "bold")).grid(
        ...
        tk.Label(card, text="Username", bg=WHITE, font=("Arial", 11, "bold")).grid(
            row=2, column=0, sticky="w", pady=8
        )
        username = tk.Entry(card, width=32, font=("Arial", 11), relief="solid", bd=1)
        username.grid(row=2, column=1, pady=8, ipady=7)

        tk.Label(card, text="Password", bg=WHITE, font=("Arial", 11, "bold")).grid(
    ...
    def show_dashboard(self):
        ...
        tk.Label(sidebar, text="RMS", bg=MAROON, fg="white", font=("Arial", 26, "bold")).pack(
            pady=(28, 5)
        )
        tk.Label(
            sidebar,
        ...
        items = [
            ...
            ("🧾  Create Order", self.orders_window),
        ...
        if role == "admin":
            items.extend([
                ...
                ("🕒  Staff Logs", self.attendance_logs_window),
            ])
        else:
            items.append(("⏱  Clock In / Out", self.clock_window))
        ...
        tk.Label(
            header,
            text="Welcome, " + self.current_user["username"].title(),
            bg=BG,
            fg=DARK,
            font=("Arial", 12, "bold")
        ).pack(side="right")
        ...
        self.button(main, "Refresh Dashboard", self.show_dashboard, 18, BLUE).pack(pady=10)

        footer = tk.Frame(main, bg="#edf0f5", height=34)
        footer.pack(side="bottom", fill="x")

        footer_text = "Role-based access enabled"
        if role == "admin":
            footer_text += " | Admin can view reports and staff logs"
        else:
            footer_text += " | Staff can take orders and clock in/out"

        tk.Label(footer, text=footer_text, bg="#edf0f5", fg=GRAY, font=("Arial", 9)).pack(
            side="left", padx=20
        )
    ...
    def stat_card(self, parent, title, value, icon, col):
        ...
        tk.Label(box, text=str(value), bg=WHITE, fg=MAROON, font=("Arial", 18, "bold")).pack(
            anchor="w"
        )
    ...
    def crud_window(self, title, fields, table, pk):
        ...
        tk.Label(form, text=title, bg=WHITE, fg=MAROON, font=("Arial", 16, "bold")).pack(
            pady=(0, 18)
        )
        ...
        tk.Label(toolbar, text=f"{title} List", bg=BG, fg=DARK, font=("Arial", 14, "bold")).pack(
            side="left"
        )
    ...
    def orders_window(self):
        ...
        tk.Label(left, text="Create Order", bg=WHITE, fg=MAROON, font=("Arial", 17, "bold")).pack(
            pady=(0, 15)
        )

        customers = self.db.query("SELECT customer_id, name FROM customers ORDER BY name", fetch=True) or []
        tables = self.db.query(
            "SELECT table_id, table_number FROM restaurant_tables WHERE status IN ('Available','Reserved') ORDER BY table_number",
            fetch=True
        ) or []
        items = self.db.query(
            "SELECT item_id, item_name, price FROM menu_items WHERE availability='Available' ORDER BY item_name",
            fetch=True
        ) or []
        ...
        for txt in ["Customer", "Table", "Item"]:
            tk.Label(left, text=txt, bg=WHITE, fg=DARK, font=("Arial", 10, "bold")).pack(
                anchor="w", pady=(8, 3)
            )
            ...
            if txt == "Customer":
                customer_cb = cb
                cb["values"] = list(customer_map.keys())
            elif txt == "Table":
                table_cb = cb
                cb["values"] = list(table_map.keys())
            else:
                item_cb = cb
                cb["values"] = list(item_map.keys())

            cb.pack(ipady=4)

        tk.Label(left, text="Quantity", bg=WHITE, fg=DARK, font=("Arial", 10, "bold")).pack(
            anchor="w", pady=(8, 3)
        )
        ...
        tk.Label(right, text="Order Items", bg=BG, fg=DARK, font=("Arial", 15, "bold")).pack(
            anchor="w", pady=(0, 10)
        )
        ...
        tk.Label(right, textvariable=total_var, bg=BG, fg=MAROON, font=("Arial", 16, "bold")).pack(
            anchor="e", pady=12
        )
        ...
        def save_order():
            ...
            order_id = self.db.query(
                "INSERT INTO orders(customer_id, table_id, created_by) VALUES(%s,%s,%s)",
                (
                    customer_map[customer_cb.get()],
                    table_map[table_cb.get()],
            ...
            for r in tree.get_children():
                ...
                self.db.query(
                    ...
                    (order_id, v[0], v[3], v[2])
                )

            self.db.query(
                "UPDATE restaurant_tables SET status='Occupied' WHERE table_id=%s",
                (table_map[table_cb.get()],)
            )
        ...
        self.button(left, "Save Order", save_order, 20, BLUE).pack(pady=(20, 5))
    ...
    def current_orders_window(self):
        ...
        tk.Label(header, text="Current Orders", bg=BG, fg=MAROON, font=("Arial", 20, "bold")).pack(
            side="left"
        )

        cols = ["order_id", "order_date", "customer", "table_number", "created_by", "items", "subtotal", "status"]
        ...
        def load():
            ...
            rows = self.db.query("""
                ...
                GROUP BY o.order_id, o.order_date, c.name, t.table_number, u.username, o.table_id, o.status
            ...
            for r in rows:
                tree.insert(
                    ...
                    values=[
                        ...
                        r["created_by"],
                        r["items"],
                        f"${Decimal(str(r['subtotal'] or 0)):.2f}",
                        r["status"]
                    ]
                )
        ...
        def mark_served():
            ...
            messagebox.showinfo("Served", f"Order #{order_id} marked as served.")
            load()

        self.button(header, "Refresh", load, 10, GRAY).pack(side="right")
        self.button(buttons, "Mark Served", mark_served, 15, BLUE).grid(row=0, column=0, padx=8)
    ...
    def billing_window(self):
        ...
        orders = self.db.query(
            "SELECT order_id FROM orders WHERE status IN ('Open','Served') ORDER BY order_id DESC",
            fetch=True
        ) or []

        order_cb = ttk.Combobox(top, values=[o["order_id"] for o in orders], state="readonly", width=15)

        tk.Label(top, text="Order ID:", bg=WHITE, fg=DARK, font=("Arial", 12, "bold")).pack(
            side="left", padx=(0, 10)
        )
        ...
        def load_bill():
            ...
            for r in rows:
                ...
                detail.insert(
                    tk.END,
                    f"{r['item_name']:<25} x{r['quantity']:<3} @ ${Decimal(str(r['unit_price'])):>7.2f} = ${line:>7.2f}\n"
                )
        ...
        def confirm_payment():
            ...
            self.db.query(
                ...
                (order_cb.get(), subtotal, tax, disc, final, payment.get())
            )
    ...
    def clock_window(self):
        ...
        def load_status():
            rows = self.db.query("""
                ...
                WHERE user_id=%s AND status='Clocked In' AND clock_out_time IS NULL
                ORDER BY log_id DESC LIMIT 1
            """, (self.current_user["user_id"],), fetch=True) or []

            if rows:
                status_var.set(f"Currently Clocked In\nClock In Time: {rows[0]['clock_in_time']}")
            else:
                status_var.set("Currently Clocked Out")
        ...
        def clock_in():
            ...
            self.db.query("INSERT INTO attendance_logs(user_id) VALUES(%s)", (self.current_user["user_id"],))
            messagebox.showinfo("Clocked In", "Clock-in recorded successfully.")
            load_status()
        ...
        def clock_out():
            ...
            self.db.query("""
                ...
                SET clock_out_time=NOW(), status='Clocked Out'
                WHERE log_id=%s
            """, (rows[0]["log_id"],))

            messagebox.showinfo("Clocked Out", "Clock-out recorded successfully.")
            load_status()
        ...
        self.button(buttons, "Clock In", clock_in, 14, BLUE).grid(row=0, column=0, padx=10)
        self.button(buttons, "Clock Out", clock_out, 14, RED).grid(row=0, column=1, padx=10)

        load_status()

    def attendance_logs_window(self):
        if self.current_user["role"].lower() != "admin":
            messagebox.showerror("Access Denied", "Only admin users can view staff logs.")
            return

        win = tk.Toplevel(self.root)
        win.title("Staff Attendance Logs")
        ...
        cols = ["log_id", "username", "role", "clock_in_time", "clock_out_time", "status"]
        ...
        def load():
            ...
            for r in rows:
                tree.insert("", "end", values=[
                    ...
                    r["role"],
                    r["clock_in_time"],
                    r["clock_out_time"] or "Not clocked out",
                    r["status"]
                ])
    ...
    def reports_window(self):
        ...
        text = tk.Text(win, height=27, width=112, relief="solid", bd=1, font=("Consolas", 10))
        text.pack(padx=20, pady=10)

        total = self.db.query(
            "SELECT COALESCE(SUM(final_amount),0) AS total_sales, COUNT(*) AS total_orders FROM bills",
            fetch=True
        )[0]
        ...
        for r in top_items:
            text.insert(
                tk.END,
                f"- {r['item_name']}: {r['qty']} sold, ${Decimal(str(r['sales'])):.2f}\n"
            )
        ...
        for r in history:
            ...
            text.insert(
                tk.END,
                f"#{r['order_id']} | {r['order_date']} | Customer: {r['customer']} | Staff: {r['created_by']} | ${Decimal(str(amount)):.2f} | {r['status']}\n"
            )
